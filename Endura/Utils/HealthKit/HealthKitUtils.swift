        ...
        for route in routes {
            ...
            for point in data {
                ...
                for i in 0..<heartRate.count {
                    if Int(heartRate[i].timestamp.timeIntervalSince1970) == Int(point.timestamp.timeIntervalSince1970) { // Check if dates are about equal
                        heartRateAtPoint = heartRate[i].heartRate
